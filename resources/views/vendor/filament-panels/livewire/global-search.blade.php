@php
    ...
    // Helper fungsi penanda warna stabilo untuk kata yang cocok
    $searchTerms = array_values(array_filter(preg_split('/\s+/u', trim($search ?? '')) ?: []));
    $highlight = function (mixed $text) use ($searchTerms): \Illuminate\Support\HtmlString {
        if (blank($text)) {
            return new \Illuminate\Support\HtmlString('');
        }

        $escapedText = ($text instanceof \Illuminate\Contracts\Support\Htmlable)
            ? $text->toHtml()
            : e((string) $text);
        ...
        // Susun regex case-insensitive aman dari karakter khusus
        $quotedTerms = array_map(fn ($term) => preg_quote(e($term), '/'), $searchTerms);
        $pattern = '/(' . implode('|', $quotedTerms) . ')/iu';

        $highlighted = preg_replace(
            $pattern,
            '<mark class="fi-stabilo-highlight">$1</mark>',
            $escapedText
        );
